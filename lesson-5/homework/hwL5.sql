create database f44_Homework_L5
go
use f44_Homework_l5
...
select Productname as MahsulotNomi
from Products
...
CREATE TABLE Customers (
    ...
    Country VARCHAR(100)
);select FirstName as Ism,LastName as Familiya
from Customers
...
select iif(StockQuantity>100, 'Yes', 'No') as natija from Products_Discounted
select ProductName from Products
except
select ProductName from Products_Discounted
...
select *,
 Case
	 ...
	 when Quantity between 2 and 3 then '7%'
  end as Discountpercent,
Case
 	 when Quantity = 1 then TotalAmount * 0.97
	 when Quantity > 3 then TotalAmount * 0.93
	 when Quantity between 2 and 3 then TotalAmount * 0.95
 end as WithDiscount
  from Orders
...
Where
 case
	when age > 25 or salary > 60000 then 1
	else 0
	end = 1
select * from Employees

select iif(employeeID = 5, salary * 1.1, iif(DepartmentName = 'HR', salary * 1.1, salary)) as NewSalary from Employees
