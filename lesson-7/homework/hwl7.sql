create database f44_Homework_l7
go
use f44_Homework_L7
...
select * from Orders
select CustomerID,
count( distinct orderdate) as zakazsoni
from orders
group by CustomerID

select OrderDate,
count( distinct ProductID) as Tovarsoni
from orders
group by OrderDate

select MONTH(orderdate) as monthnumber,
sum(TotalAmount) as totalsales,
count( distinct ProductID) as countsuniqueproducts
from Orders
group by MONTH(orderdate)
having count( distinct ProductID) >= 2

select year(orderdate) as yearnumber,
min(OrderID) as minorder,
...
select * from Sales
select ProductID, sum(saleamount) as totalsalesamount from Sales
group by ProductID
having ProductID = 7
...
select ProductID,
 ...
 select Year(saledate) as yearnumber,
sum(saleamount) as totalsales
from Sales
